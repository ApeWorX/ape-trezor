...
class HDPath:
    ...
    for both account specific HD paths (:class:`~ape_trezor.hdpath.HDAccountPath`)
...
class HDAccountPath(HDPath):
    ...
    def as_bytes(self):
        """
        Convert ``self.path`` to the Trezor bytes format.
        Trezor expect the following bytes input:
...
class HDBasePath(HDPath):
    ...
    :class:`~ape_trezor.hdpath.HDAccountPath`.
    """
    ...
    def get_account_path(self, account_id) -> HDAccountPath:
        return HDAccountPath(f"{self.path}/{str(account_id)}")
